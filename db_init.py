...
'''
    # Create spots table (with lot_id)
    c.execute("""
        CREATE TABLE spots (
            spot_id VARCHAR(20),
            level INT,
    ...
    # Create tickets table (with lot_id)
    c.execute("""
        CREATE TABLE tickets (
            ticket_id VARCHAR(36) PRIMARY KEY,
            spot_id VARCHAR(20),
    ...
    # Insert some free spots for multiple lots
    spots = []
    lots = ["LOT1", "LOT2"]   

    for lot in lots:
        for level in range(1, 3):      #levels
            for number in range(1, 6): #spots
                spot_id = f"L{level}-S{number}"
                spots.append((spot_id, level, number, "free", lot))

    c.executemany("INSERT INTO spots (spot_id, level, number, status, lot_id) VALUES (%s, %s, %s, %s, %s)", spots)

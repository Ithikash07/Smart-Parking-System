...
class ParkingService:
    def enter(self, license_plate, lot_id, idem_key=None):
        # 🔹 Step 1: Idempotency check
        if idem_key:
            cached = idempotency_repo.get_response(idem_key)
            if cached:
        ...
        # 🔹 Step 2: Prevent same car in same lot
        existing_ticket = ticket_repo.find_active_ticket_by_plate(license_plate, lot_id)
        if existing_ticket:
            response = {
                "ticket_id": existing_ticket["ticket_id"],
        ...
        # 🔹 Step 3: Find a free spot in that lot
        spot = spot_repo.find_free_spot(lot_id)
        if not spot:
            return None, f"No spots available in {lot_id}"

        spot_id = spot["spot_id"]

        # 🔹 Step 4: Occupy spot
        spot_repo.occupy_spot(spot_id, lot_id)

        # 🔹 Step 5: Create new ticket
        ticket_id = str(uuid.uuid4())
        in_time = int(time.time())
        ticket_repo.create_ticket(ticket_id, spot_id, license_plate, in_time, lot_id)
